...
namespace RoslynTool.CsToLua
{
    ...
    {
        public static ExitCode Process(string srcFile, IList<string> macros, IList<string> ignoredPath, IList<string> externPath, IList<string> internPath, IDictionary<string, string> _refByNames, IDictionary<string, string> _refByPaths, bool enableInherit, bool enableLinq, bool outputResult, bool parallel)
        {
...
#pragma warning restore 618
            ...
            foreach (var pair in toplevelClasses) {
                ...
                foreach (string lib in dsllibRefs) {
                    string libFile = Path.Combine(exepath, "dsllib/" + lib);
                    if (File.Exists(libFile)) {
                        File.Copy(libFile, Path.Combine(outputDir, string.Format("{0}.{1}", lib, c_OutputExt)), true);
                    } else {
                        Console.WriteLine("Can't find file '{0}' in dsllib, you should copy it to output dir !", libFile);
                    }
                }
                File.WriteAllText(Path.Combine(outputDir, Path.ChangeExtension(fileName, c_OutputExt)), classBuilder.ToString());
            }
        ...
        private static void BuildClassAttributes(StringBuilder sb, INamedTypeSymbol typesym, IAssemblySymbol assemblySymbol, Dictionary<string, INamedTypeSymbol> ignoredClasses)
        {
            ...
            foreach (var sym in typesym.GetMembers()) {
                ...
                if (null != msym && msym.GetAttributes().Length > 0 && !ClassInfo.HasAttribute(sym, "Cs2Dsl.IgnoreAttribute")) {
                    temp.AppendFormat("{0}method({1}){{", GetIndentString(indent), SymbolTable.CalcMethodMangling(msym));
                    temp.AppendLine();
                    ++indent;
                    BuildAttributes(temp, indent, msym.GetAttributes());
                    --indent;
            ...
            --indent;
            if (csb.Length > 0) {
                sb.AppendFormat("attributes({0}){{", ClassInfo.GetFullName(typesym).Replace(".", "__"));
                sb.AppendLine();
