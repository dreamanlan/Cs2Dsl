        ...
        static void Main(string[] args)
        {
            try {
                ...
                if (args.Length > 0) {
                    ...
                    Console.WriteLine("\t\tignorepath = ignore c# source file path in the csproj, these classes doesn't translate to dsl (need translate them by hand, cs2dsl use \"require 'cs2dsl_custom';\" resolve xref).");
                ...
                if (File.Exists(file)) {
                    ...
                    if (genLua) {
                        ...
                        Generator.JsGenerator.Generate(Path.GetDirectoryName(file), outputDir, outputExt, parallel);
                    }
					stopwatch2.Stop();
                    Console.WriteLine("Generator consume time: {0}s", stopwatch2.Elapsed.TotalSeconds);
